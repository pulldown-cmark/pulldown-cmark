...
fn {}_test_{i}() {{
    ...
    test_markdown_html(original, expected, {smart_punct}, {standard_footnotes});
}}
"###,
                    spec_name,
                    i = i + 1,
                    ...
                    standard_footnotes = testcase.standard_footnotes,
                ))
                .unwrap();
...
impl<'a> Iterator for Spec<'a> {
    ...
    fn next(&mut self) -> Option<TestCase> {
        ...
        let (i_start, smart_punct, standard_footnotes) =
            self.spec.find(prefix).and_then(|pos| {
                let punct_suffix = "_smartpunct\n";
                let footnote_suffix = "_standard_footnotes\n";
                if spec[(pos + prefix.len())..].starts_with(punct_suffix) {
                    Some((pos + prefix.len() + punct_suffix.len(), true, false))
                } else if spec[(pos + prefix.len())..].starts_with(footnote_suffix) {
                    Some((pos + prefix.len() + footnote_suffix.len(), false, true))
